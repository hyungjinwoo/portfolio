...
<%
	...
	List<PlaceList> placeList = (List<PlaceList>)request.getAttribute("placeList");
	
	//일정 길이 채크
	Set<Integer> set = new HashSet<>();
	for(PlaceList pl : placeList){
		set.add(pl.getDay());
	}
...
<script>
$(function(){
	var startDate = "<%=plan.getStartDate()%>".split("-");
	console.log(startDate);
	var startDay = new Date(startDate[0],startDate[1],startDate[2]);
	console.log(startDay);
	
	var endDate = "<%=plan.getEndDate()%>".split("-");
	console.log(endDate);
	var endDay = Number(new Date(endDate[0],endDate[1],endDate[2]));
	console.log(endDay);
...
/* 			console.log(placeArr[j]); */
	...
	$.ajax({
		url:"<%=request.getContextPath()%>/plan/planRewriteEnd?planNo=<%=plan.getPlanNo()%>",
...
<input type="text" id="planTitle" value="<%=plan.getPlanTitle()%>" placeholder="제목을 입력해주세요."/>
...
<div id="planWriteFrm-container">
	...
	<div id="planFrm-container">
		<div id="planWriteList">
			...
			<ul id='drag-ul' class='list-group list-group-sortable-connected'>
				<p id='day' draggable='false'><%=i+1%>일차</p>
				<%for(PlaceList pl : placeList){ %>
					<%if(i+1 == pl.getDay()){ %>
				<li class="list-group-item list-group-item-info" draggable="true">
					<input type='hidden' id='placeName' value='<%=pl.getPlaceName()%>'/>
					<input type='hidden' id='placeLat' value='<%=pl.getPlaceLat()%>'/>
					<input type='hidden' id='placeLng' value='<%=pl.getPlaceLng()%>'/>
					<span id="wishPlaceName"><%=pl.getPlaceName()%></span>
					<img src='<%=request.getContextPath()%>/images/plan-memo.png' id='inputPlaceMemo' onclick='memoView(this);'/>
					<textarea id='placeMemo'><%=pl.getPlaceComment()==null?"":pl.getPlaceComment()%></textarea>
					<button id='delPlace' onclick='delPlace(this);'>삭제</button>
				</li>
					<%} %>
				<%} %>
	...
	<button id="planList-input-btn" onclick="planListInput();">수정하기</button>

<!--히든태그-->
<input type="hidden" id="startDay" value="<%=plan.getStartDate()%>"/>
<input type="hidden" id="endDay" value="<%=plan.getEndDate()%>"/>
<input type="hidden" id="readCount" value="<%=plan.getReadCount()%>"/>
<input type="hidden" id="likeCount" value="<%=plan.getLikeCount()%>"/>
<input type="hidden" id="planLevel" value="<%=plan.getPlanLevel()%>"/>
</section>
